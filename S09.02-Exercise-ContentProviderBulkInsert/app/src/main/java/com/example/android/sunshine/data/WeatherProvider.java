...
//  COMPLETED Implement the bulkInsert method
    /**
...
//          COMPLETED Only perform our implementation of bulkInsert if the URI matches the CODE_WEATHER code
            case CODE_WEATHER :
                int rowsInserted = 0;
                ...
                try {
                    for (ContentValues value : values) {
                        ...
                        if (!SunshineDateUtils.isDateNormalized(weatherDate)) {
                            throw new IllegalArgumentException("Date is not normalized! " + weatherDate);
                        }
                        long dataId = wDb.insert(WeatherContract.WeatherEntry.TABLE_NAME, null, value);
                        if (dataId != -1) {
                            rowsInserted++;
                        }
                    }
...
//              COMPLETED Return the number of rows inserted from our implementation of bulkInsert
                return rowsInserted;
//          COMPLETED If the URI does match match CODE_WEATHER, return the super implementation of bulkInsert
            default:
    ...
    public int delete(@NonNull Uri uri, String selection, String[] selectionArgs) {
        SQLiteDatabase wDb = mOpenHelper.getWritableDatabase();
        int deletedRecord = wDb.delete(WeatherContract.WeatherEntry.TABLE_NAME, selection, selectionArgs);
        getContext().getContentResolver().notifyChange(uri, null);
